...
describe Kitchen::PlatformFilter do
  describe ".convert" do
    it "returns an array if a scalar is passed" do
      _(Kitchen::PlatformFilter.convert(nil)).must_equal []
      _(Kitchen::PlatformFilter.convert("string")).must_equal [Kitchen::PlatformFilter.new("string")]
    end

    it "just wraps simple strings" do
      _(Kitchen::PlatformFilter.convert(%w{CentOS windows-2016})).must_equal [Kitchen::PlatformFilter.new("CentOS"),
                                                                           Kitchen::PlatformFilter.new("windows-2016")]
    end

    it "converts regexp-like strings into Regexp before wrapping" do
      _(Kitchen::PlatformFilter.convert(%w{/^win/ /win$/})).must_equal [Kitchen::PlatformFilter.new(/^win/),
                                                                     Kitchen::PlatformFilter.new(/win$/)]
    end

    it "supports IgnoreCase Regexp option" do
      _(Kitchen::PlatformFilter.convert("/win/i")).must_equal [Kitchen::PlatformFilter.new(/win/i)]
    end

    it "supports Extended Regexp option" do
      _(Kitchen::PlatformFilter.convert("/win/x")).must_equal [Kitchen::PlatformFilter.new(/win/x)]
    end
  ...
  describe "#value" do
    it "returns the original value" do
      _(Kitchen::PlatformFilter.new("string").value).must_equal "string"
      _(Kitchen::PlatformFilter.new(/regexp/).value).must_equal(/regexp/)
    end
  end
end

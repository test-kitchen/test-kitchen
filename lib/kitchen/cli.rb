...
module Kitchen
  ...
  class CLI < Thor
    ...
    def update_config!
      ...
      @config.log_overwrite = options[:log_overwrite] unless options[:log_overwrite].nil?
    ...
    def update_parallel!
      if options[:parallel]
        ...
        options[:concurrency] = MAX_CONCURRENCY if options[:parallel] && !options[:concurrency]

    ...
    class ChefBase < Base
      ...
      def prepare_cookbooks
        ...
        if File.exists?(berksfile) || File.exists?(cheffile)
          debug("This is a chefrepo checking if to use berkshelf or librarian")
          ...
          # copy additional cookbooks from cookbooks and site-cookbooks
        ...
        # not a chefrepo so check if lone cookbook
          ...
          cp_this_cookbook
        # not a cookbook so make a fake one
        else
          make_fake_cookbook
        end
